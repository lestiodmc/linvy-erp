...
class ItemController extends ResourceController
{
    ...
    protected array $columns = ['sku', 'name', 'type', 'defaultWarehouseType.name', 'is_stock_item', 'is_active'];
    protected array $rules = ['sku' => ['required', 'string', 'max:255'], 'name' => ['required', 'string', 'max:255'], 'type' => ['required', 'string'], 'item_category_id' => ['required', 'integer'], 'unit_of_measure_id' => ['required', 'integer'], 'default_warehouse_type_id' => ['nullable', 'exists:warehouse_types,id'], 'is_stock_item' => ['nullable'], 'standard_cost' => ['required', 'numeric'], 'cost_method' => ['required', 'string'], 'use_category_default_accounts' => ['nullable'], 'inventory_account_id' => ['nullable', 'integer'], 'cogs_account_id' => ['nullable', 'integer'], 'sales_account_id' => ['nullable', 'integer'], 'purchase_account_id' => ['nullable', 'integer'], 'wip_account_id' => ['nullable', 'integer'], 'adjustment_account_id' => ['nullable', 'integer'], 'waste_account_id' => ['nullable', 'integer'], 'is_active' => ['nullable'], 'notes' => ['nullable', 'string']];

    public function __construct()
    {
        $categories = ItemCategory::orderBy('name')->pluck('name', 'id')->toArray();
        $uoms = UnitOfMeasure::orderBy('name')->pluck('name', 'id')->toArray();
        $warehouseTypes = WarehouseType::where('is_active', true)->orderBy('name')->pluck('name', 'id')->toArray();
