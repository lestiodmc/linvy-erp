...
class SalesOrderController extends ResourceController
{
    ...
    protected array $rules = ['number' => ['required', 'string', 'max:255'], 'customer_id' => ['required', 'integer'], 'order_date' => ['required', 'date'], 'requested_delivery_date' => ['nullable', 'date'], 'status' => ['required', 'string'], 'subtotal' => ['required', 'numeric'], 'tax_total' => ['required', 'numeric'], 'grand_total' => ['required', 'numeric'], 'notes' => ['nullable', 'string']];
