<x-app-layout>
    <x-slot name="header">
        <x-ui.page-header
            title="Stock Balances"
            subtitle="Current inventory by item, branch, and warehouse."
        />
    </x-slot>

    @php
        $formatQty = fn ($value) => number_format((float) $value, 2);
        $formatMoney = fn ($value) => 'Rp'.number_format((float) $value, 2, ',', '.');
        $formatDate = fn ($date) => $date ? \Illuminate\Support\Carbon::parse($date)->format('d M Y') : '-';
        $badge = 'inline-flex max-w-36 truncate rounded-full px-2 py-0.5 text-[11px] font-bold ring-1 bg-slate-100 text-slate-700 ring-slate-200';
        $warehouseTypeBadge = 'inline-flex max-w-32 truncate rounded-full px-2 py-0.5 text-[10px] font-bold ring-1 bg-indigo-50 text-indigo-700 ring-indigo-100';
        $cell = 'px-3 py-2';
        $headCell = 'px-3 py-2 text-left text-[11px] font-black uppercase tracking-wide text-slate-500';
        $onHand = function ($record): float {
            $newQty = (float) ($record->qty_on_hand ?? 0);
            $legacyQty = (float) ($record->quantity_on_hand ?? 0);

            return $newQty !== 0.0 ? $newQty : $legacyQty;
        };
        $reserved = function ($record): float {
            $newQty = (float) ($record->qty_reserved ?? 0);
            $legacyQty = (float) ($record->quantity_reserved ?? 0);

            return $newQty !== 0.0 ? $newQty : $legacyQty;
        };
        $available = fn ($record) => $onHand($record) - $reserved($record);
        $availableClass = fn ($value) => $value > 0 ? 'text-emerald-700' : ($value < 0 ? 'text-red-700' : 'text-slate-400');
    @endphp

    <div class="mx-auto max-w-screen-2xl">
        <x-ui.filter-toolbar
            :action="route('stock-balances.index')"
            columns="lg:grid-cols-[minmax(13rem,1.4fr)_minmax(9rem,1fr)_minmax(9rem,1fr)_minmax(10rem,1fr)_minmax(10rem,1fr)_minmax(10rem,1fr)_7rem_6rem]"
        >
            <x-ui.search-input :value="$filters['keyword'] ?? ''" />
            <x-ui.select-filter name="company_id" label="Company" :value="$filters['company_id'] ?? ''" :options="$companies" all-label="All companies" />
            <x-ui.select-filter name="branch_id" label="Branch" :value="$filters['branch_id'] ?? ''" :options="$branches" all-label="All branches" />
            <x-ui.select-filter name="warehouse_id" label="Warehouse" :value="$filters['warehouse_id'] ?? ''" :options="$warehouses" all-label="All warehouses" />
            <x-ui.select-filter name="item_category_id" label="Item Category" :value="$filters['item_category_id'] ?? ''" :options="$itemCategories" all-label="All categories" />
            <x-ui.select-filter name="stock_status" label="Stock Status" :value="$filters['stock_status'] ?? ''" :options="$stockStatuses" all-label="All Stock" />
            <button class="h-10 rounded-lg bg-emerald-600 px-3 text-sm font-bold text-white hover:bg-emerald-700">Apply</button>
            <a href="{{ route('stock-balances.index') }}" class="flex h-10 items-center justify-center rounded-lg border border-slate-300 bg-white px-3 text-sm font-bold text-slate-700 hover:bg-slate-50">Reset</a>
        </x-ui.filter-toolbar>

        <x-ui.data-table class="rounded-lg shadow-none">
            <x-slot:head>
                <tr>
                    <th class="{{ $headCell }} whitespace-nowrap">SKU</th>
                    <th class="{{ $headCell }} min-w-56">Item</th>
                    <th class="{{ $headCell }} whitespace-nowrap">Company</th>
                    <th class="{{ $headCell }} whitespace-nowrap">Branch</th>
                    <th class="{{ $headCell }} whitespace-nowrap">Warehouse</th>
                    <th class="{{ $headCell }} whitespace-nowrap text-right">On Hand</th>
                    <th class="{{ $headCell }} whitespace-nowrap text-right">Reserved</th>
                    <th class="{{ $headCell }} whitespace-nowrap text-right">Available</th>
                    <th class="{{ $headCell }} whitespace-nowrap text-right">Average Cost</th>
                    <th class="{{ $headCell }} whitespace-nowrap">Last Movement</th>
                    <th class="{{ $headCell }} whitespace-nowrap text-right">Action</th>
                </tr>
            </x-slot:head>

            @forelse($records as $record)
                @php
                    $availableQty = $available($record);
                    $warehouseType = $record->warehouse?->warehouseType?->name;
                @endphp
                <tr class="text-xs hover:bg-slate-50">
                    <td class="{{ $cell }} whitespace-nowrap font-bold text-slate-900">{{ $record->item?->sku ?: '-' }}</td>
                    <td class="{{ $cell }} min-w-56 max-w-80 truncate text-slate-700">{{ $record->item?->name ?: '-' }}</td>
                    <td class="{{ $cell }} whitespace-nowrap"><span class="{{ $badge }}">{{ $record->company?->name ?: $record->warehouse?->company?->name ?: '-' }}</span></td>
                    <td class="{{ $cell }} whitespace-nowrap"><span class="{{ $badge }}">{{ $record->branch?->name ?: $record->warehouse?->branch?->name ?: '-' }}</span></td>
                    <td class="{{ $cell }} whitespace-nowrap">
                        <span class="{{ $badge }}">{{ $record->warehouse?->name ?: '-' }}</span>
                        @if($warehouseType)
                            <span class="{{ $warehouseTypeBadge }} ml-1">{{ $warehouseType }}</span>
                        @endif
                    </td>
                    <td class="{{ $cell }} whitespace-nowrap text-right font-semibold text-slate-900">{{ $formatQty($onHand($record)) }}</td>
                    <td class="{{ $cell }} whitespace-nowrap text-right text-slate-600">{{ $formatQty($reserved($record)) }}</td>
                    <td class="{{ $cell }} whitespace-nowrap text-right font-semibold {{ $availableClass($availableQty) }}">{{ $formatQty($availableQty) }}</td>
                    <td class="{{ $cell }} whitespace-nowrap text-right text-slate-600">{{ $formatMoney($record->average_cost) }}</td>
                    <td class="{{ $cell }} whitespace-nowrap text-slate-600">{{ $formatDate($record->last_movement_at) }}</td>
                    <td class="{{ $cell }} whitespace-nowrap text-right">
                        <a href="{{ route('stock-balances.show', $record) }}" class="inline-flex h-7 items-center rounded-md border border-slate-200 bg-white px-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50">Open</a>
                    </td>
                </tr>
            @empty
                <x-ui.empty-state colspan="11" message="No stock balances found." />
            @endforelse
        </x-ui.data-table>

        <x-ui.pagination :records="$records" />
    </div>
</x-app-layout>
