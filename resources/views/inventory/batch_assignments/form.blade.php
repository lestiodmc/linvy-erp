<x-app-layout>
    ...
    <form method="POST" action="{{ $record->exists?route('batch-assignments.update',$record):route('batch-assignments.store') }}" class="enterprise-form mx-auto max-w-screen-xl space-y-3" data-batch-form>@csrf @if($record->exists)@method('PUT')@endif
        ...
        <div class="rounded-lg border bg-white p-3"><label class="text-xs font-bold">Notes</label><textarea name="notes" class="mt-1 w-full rounded-lg border-slate-200">{{ old('notes',$record->notes) }}</textarea></div><div class="enterprise-action-bar sticky bottom-0 z-20"><a href="{{ route('batch-assignments.index') }}" class="rounded-lg border px-4 py-2 text-sm font-bold">Cancel</a><button name="action" value="draft" class="button-primary">Save Draft</button><button name="action" value="post" class="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-bold text-white">Save & Post</button></div>
    </form>
    <script>
