<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="mt-0.5 text-sm font-medium text-slate-500">Manage customer profile, addresses, sales defaults, and account status.</p>
        </div>
    </x-slot>

    @php
        $sections = [
            'General' => ['code', 'name', 'customer_group', 'customer_type', 'tax_number', 'contact_person'],
            'Contact' => ['phone', 'mobile', 'email', 'website'],
            'Billing Address' => ['billing_address', 'billing_city', 'billing_province', 'billing_country', 'billing_postal_code'],
            'Shipping Address' => ['shipping_address', 'shipping_city', 'shipping_province', 'shipping_country', 'shipping_postal_code'],
            'Sales' => ['default_currency_id', 'payment_term_id', 'default_tax_id', 'credit_limit', 'salesman', 'price_level', 'ar_account_id'],
            'Status' => ['blocked_sales', 'is_active'],
        ];
    @endphp
    ...
    <div class="mx-auto max-w-6xl">
        <form method="POST" action="{{ $action }}" class="enterprise-form overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
            ...
            <div class="border-b border-slate-100 px-6 py-5">
                <h3 class="text-base font-black text-slate-950">Customer Information</h3>
                <p class="mt-1 text-sm text-slate-500">Fill the master data used by sales and future accounting integration.</p>
            </div>
